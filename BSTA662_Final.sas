...
proc lifetest data=pneumon method=km plots=(survival(cl),ls,lls)
	...
	strata race; /* test on the difference of race */
run;

proc lifetest data=pneumon method=km plots=(survival(cl),ls,lls)
	graphics outsurv=a;
	time agepn*hospital(0);
	strata nsibs_fct; /* test on the difference of nsibs_fct */
run;
...
proc gplot data=pred;
	title "Predicted curve for a child with nsibs=0, wmonth=6, smoke=0";
	symbol1 value=dot i=join;
